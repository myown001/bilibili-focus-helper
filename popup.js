...
function loadStudyStats(period = 'week') {
  ...
  // 显示加载状态
  document.getElementById('total-time').textContent = '加载中...';
  ...
  chrome.runtime.sendMessage(
    ...
    function(response) {
      ...
      if (response && response.success) {
        ...
        document.getElementById('avg-rate').textContent = '1.0x';
  ...
  chrome.runtime.sendMessage(
    ...
    function(response) {
      if (response && response.success) {
        ...
        document.getElementById('load-more-btn').style.display = hasMoreHistory ? 'block' : 'none';
      } else {
...
function renderStats(stats) {
  ...
  totalTimeElement.textContent = formatTime(stats.totalDuration);
  
  // 更新学习视频数
  const videoCountElement = document.getElementById('video-count');
  videoCountElement.textContent = stats.videoCount;
